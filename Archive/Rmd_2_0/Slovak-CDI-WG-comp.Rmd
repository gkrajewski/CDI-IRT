---
title: "Slovak CDI:WG comprehension analysis"
...
read_csv("Data/slovak_wb_wg.csv") %>%
  filter(item_kind == "word") %>%
  select(child_id, item_id, item_definition, english_gloss, value) -> responses

# We get items (Slovak, English glosses, and IDs):
...
read_csv("Data/slovak_wb_participants.csv") %>%
  filter(form == "WG") %>%
...
```{r quadpts_comp2, eval=FALSE}
sapply(1:ncol(quadpts_comp), function(x) quadpts_comp[,x]$item1pars)
```

Based on the above we set the number of quadrature points to 601.
...
output <- misfits_removal(responses=responses, quadtps=quadtp, NCYCLES=100000, p=0.001,
                          output_file = "Data/slovak_wg_comp.RData")
```
...
```{r model_summary}
load("Data/slovak_wg_comp.RData")
mod <- output[[1]]
...
save(residuals, file = "Data/slovak_wg_comp_residuals.RData")
```

```{r local_dependence_histogram}
load("Data/slovak_wg_comp_residuals.RData")
...
There are quite a few but following Kachergis et al. we set the threshold for removing items to V > .5
...
output <- misfits_removal(responses=responses, quadtps=quadtp, NCYCLES=100000, p=0.001,
                          output_file = "Data/slovak_wg_comp_2.RData")
```
...
```{r model_summary_2}
load("Data/slovak_wg_comp_2.RData")
mod <- output[[1]]
items_removed <- output[[2]]
rm(output) # to save space
...
ggplot(scores, aes(comprehension, F1)) +
  ...
  ylim(-4, 4) +
#  labs(title = "Slovak CDI:WG Comprehension") +
  theme(
  panel.background = element_rect(fill = NA),
...
write_csv(scores, "Data/scores_slovak.csv")
```
...
save(results_all, file = "Data/slovak_wg_comp_sim_all.Rdata")
...
```{r sim_all_report}
load("Data/slovak_wg_comp_sim_all.Rdata")
...
write_csv(meanSE_item, "Data/meanSE_slovak.csv")
```


## SE = 0.1 stop criterion

The plot above shows that we need quite a number of items (ca. 100) to reach SE=.1 on average.
Let's try a simulation.

```{r se1, eval=FALSE}
results_SE1 <- sim_se(0.1, file = "Data/slovak_wg_comp_sim_se1.RData")
```


```{r se1_report}
load("Data/slovak_wg_comp_sim_se1.Rdata")
results_SE1 <- results
...
plot_length(results_SE1, "Slovak CDI:WG Comp", 0.1)
```

## SE = 0.15 stop criterion

```{r se15, eval=FALSE}
results_SE15 <- sim_se(0.15, file = "Data/slovak_wg_comp_sim_se15.RData")
```


```{r se15_report}
load("Data/slovak_wg_comp_sim_se15.Rdata")
...
plot_length(results_SE15, "Slovak CDI:WG Comp", 0.15)
```
...
prepare_input_files(params, cdi$item_definition, fscores_aggr,
                    ...
                    file1 = "items-Slovak-WG-comp.csv", file2 = "startThetas-Slovak-WG-comp.csv")
```
