...
1. IRT model (`mod`)
1. `responses` matrix (administrations x items; 0/1) used for the final model fitting
2. `cdi` data frame (items with their definitions & glosses) used for the final model fitting
3. `scores` data frame (participant information: raw scores & demographics)


```{r data_loading}
list.files(path = data_dir, pattern = "\\.RData$", full.names = TRUE) |>
