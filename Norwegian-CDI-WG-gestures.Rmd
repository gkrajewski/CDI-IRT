...
1. `responses` matrix (self explanatory)
2. `cdi` data frame (item information)
3. `scores` data frame (participant information: raw scores $ demographics)

We import responses, items, and participant data from local files,
which in turn are downloaded from WordBank, exclude non norming (longitudinal?) data,
and perform a check (our sums equal to imported scores).

```{r data}
# We get responses and item data:
# wordbankr::get_instrument_data("[language]", "WG|WS", *_info=T) %>% filter(is_norming)
# 
...
# We recode responses into zeroes and ones:
if_else(is.na(responses$value) | responses$value == "", 0, 1) -> responses$value

# We get scores and demo data.
# wordbankr::get_administration_data("[language]", include_*=T)
# 
# More demo data available, if needed: modify select():
read_csv("Data/norwegian_wb_participants.csv") %>%
  filter(form == "WS" & is_norming) %>%
  select(child_id, age, sex, production) -> scores

# We recode sex (just because we used girl/boy before):
