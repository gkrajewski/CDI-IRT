...
Relevant responses are recoded as 0's and 1's appropriately and
children with at least one `NA` are excluded.
...
# Drop children with at least one NA):
original_n <- length(unique(responses$child_id))
responses %>% group_by(child_id) %>%
  filter(!any(is.na(value))) %>% ungroup() -> responses
```
...
no_na_n <- length(unique(responses$child_id))
...
There were `r original_n` `r paste(language, scale)` administrations.
`r no_na_n` left after removing those with missing data.
There were`r original_cdi_n` `r paste(language, scale)` items.
